...
G2 = G.copy()
G2.remove_node('Wean Hall')

try:
    new_path = nx.shortest_path(G2, source=start, target=end, weight='weight')
    new_time = nx.shortest_path_length(G2, source=start, target=end, weight='weight')

    print("\nWith Wean Hall closed:")
    print(f"  New route: {' -> '.join(new_path)}")
    print(f"  New walking time: {new_time} minutes")

except nx.NetworkXNoPath:
    print("\nNo available path when Wean Hall is closed!")
